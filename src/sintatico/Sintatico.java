...
public class Sintatico {
	/**
	 * classe que CONT�M os m�todos necess�rios para a analise sint�tica, procurando as regras da gramatica
	 * *e verificando se s�o validas ou n�o
	...
	**/
	...
	public String getMsgRetornoSintatico() {
		if (!isErroSintatico())
			msgSintatico = "An�lise Sint�tica executada com sucesso!";
		return msgSintatico;
	}
	...
	//METODO QUE IMPLEMENTA A PILHA, VERIFICANDO OS TERMINAIS E N�O TERMINAIS
	public void analisarInteiro(List<Token> tokensLexico) {
		...
		for (i = 0; i < tokensLexico.size(); i++) {
			...
			if (ConstantesTerminais.TERMINAIS.containsValue(topoPilha)) {
				if (topoPilha.equals(Integer.valueOf(token.getCodigo()))) {
					...
					msgSintatico = "Erro sint�tico, gram�tica n�o reconhece sequencia de tokens\nTopo da Pilha= " +
							topoPilha + "\n" +
							"C�digo do Token esperado= " + token.getCodigo() + "\n" +
		...
		if (topoPilha.equals(Integer.valueOf(101))) {
			...
			msgSintatico = "Erro sint�tico, ainda existem elementos na pilha!";
			erro = true;
		}

	}

	//METODO QUE PROCURA A REGRA QUANDO � N�O TERMINAL
			...
			if ((this.matrizParse[i][0] != null) && (this.matrizParse[i][0].equals(naoTerminal))) {
				...
				while (j < 47) {
					...
					if (cabecalho.equals(Integer.valueOf(token.getCodigo()))) {
						...
						msgSintatico = "Erro sint�tico:\n Regra " + naoTerminal + " n�o pode ser derivada pr�ximo a: \""+semantico.getPenultimo()+"\"";
						erro = true;
					}
					j++;
				}
