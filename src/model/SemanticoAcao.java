...
//		AL_DSVS.removeAll(AL_DSVS);
		...
		try {
			...
			case 101:
				try {
					...
					lc.setOp1(0);
					lc.setOp2(0);
					view.PrincipalGUI.AL_Instr.add(lc);
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 108:
				try {
					if (Varrer(model.Semantico.A.get(model.Semantico.i).getNome())) {
						...
						v.setNivel(nvl);
						v.setGA(view.PrincipalGUI.AL_Instr.size() + 1);
						v.setGB(0);
						v.setPROX(0);
						TS.add(v);
						nvl += 1;
			...
			case 109:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc);
					n_proc += 1;
					PROCEDURE[n_proc] = view.PrincipalGUI.AL_Instr.size() - 1;
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 110:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc);
					nvl -= 1;
					view.PrincipalGUI.AL_Instr.get(PROCEDURE[n_proc]).setOp2(view.PrincipalGUI.AL_Instr.size());
				...
				try {
					...
					if (!Varrer(tkn)) {
						if ("rótulo".equals(Tipo(tkn))) {
							...
							if (TS.get(j).getGA() == 0) {
								if (j == nvl) {
									TS.get(j).setGA(view.PrincipalGUI.AL_Instr.size());
									b = true;
								} else {
									Erro(4);
								}
			...
			case 120:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc);
					n_if += 1;
					IF[n_if] = view.PrincipalGUI.AL_Instr.size() - 1;
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 121:
				try {
					view.PrincipalGUI.AL_Instr.get(IF[n_if]).setOp2(view.PrincipalGUI.AL_Instr.size());
					n_if -= 1;
					b = true;
				} catch (Exception e) {
					Erro(i);
			...
			case 123:
				try {
					n_while += 1;
					WHILE[n_while] = view.PrincipalGUI.AL_Instr.size();
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 124:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc);
					n_while += 1;
					WHILE[n_while] = (view.PrincipalGUI.AL_Instr.size() - 1);
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 125:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc);
					view.PrincipalGUI.AL_Instr.get(WHILE[n_while]).setOp2(view.PrincipalGUI.AL_Instr.size());
					n_if -= 2;
					b = true;
				} catch (Exception e) {
					Erro(i);
			...
			case 126:
				try {
					n_repeat += 1;
					REPEAT[n_repeat] = view.PrincipalGUI.AL_Instr.size();
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
					...
					if (!Varrer(temp)) {
						if (contexto.equals("readln")) {
							if ("variável".equals(Tipo(temp))) {
								...
								view.PrincipalGUI.AL_Instr.add(lc2);
								b = true;
							} else {
								Erro(3);
							}
			...
			case 130:
				try {
					...
					lc.setSeq(23);
					lc.setCod("IMPRL");
					lc.setOp1(view.PrincipalGUI.AreaLiterais.length());
					view.PrincipalGUI.AreaLiterais += temp;
					lc.setOp2(view.PrincipalGUI.AreaLiterais.length() - 1);
					view.PrincipalGUI.AL_Instr.add(lc);
					b = true;
				} catch (Exception e) {
					Erro(i);
			...
			case 133:
				try {
					for (int j = 0; j <= n_case2; j++) {
						view.PrincipalGUI.AL_Instr.get(CASE2[j]).setOp2(view.PrincipalGUI.AL_Instr.size());
					}
					n_case2 = -1;
					SemanticoInstrucao lc = new SemanticoInstrucao();
					lc.setSeq(24);
			...
			case 134:
				try {
					...
					lc.setOp2(0);
					view.PrincipalGUI.AL_Instr.add(lc);
					case_dsvs = view.PrincipalGUI.AL_Instr.size() - 1;
					for (int j = 0; j < (n_case + 1); j++) {
						view.PrincipalGUI.AL_Instr.get(CASE[j]).setOp2(view.PrincipalGUI.AL_Instr.size());
					}
					n_case = -1;
					b = true;
				} catch (Exception e) {
			...
			case 135:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc2);
					n_case2 += 1;
					CASE2[n_case2] = view.PrincipalGUI.AL_Instr.size() - 1;
					view.PrincipalGUI.AL_Instr.get(case_dsvs).setOp2(view.PrincipalGUI.AL_Instr.size());
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 136:
				try {
					...
					lc3.setOp1(0);
					lc3.setOp2(0);
					view.PrincipalGUI.AL_Instr.add(lc3);
					SemanticoInstrucao lc4 = new SemanticoInstrucao();
					lc4.setSeq(29);
					lc4.setCod("DSVT");
					lc4.setOp1(0);
					lc4.setOp2(0);
					view.PrincipalGUI.AL_Instr.add(lc4);
					n_case += 1;
					CASE[n_case] = view.PrincipalGUI.AL_Instr.size() - 1;
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 139:
				try {
					n_for += 1;
					FOR[n_for] = view.PrincipalGUI.AL_Instr.size();
					SemanticoInstrucao lc = new SemanticoInstrucao();
					lc.setSeq(28);
					lc.setCod("COPI");
					lc.setOp1(0);
					...
					view.PrincipalGUI.AL_Instr.add(lc4);
					n_for += 1;
					FOR[n_for] = view.PrincipalGUI.AL_Instr.size() - 1;
					b = true;
				} catch (Exception e) {
					Erro(i);
				}
			...
			case 140:
				try {
					...
					view.PrincipalGUI.AL_Instr.add(lc5);
					view.PrincipalGUI.AL_Instr.get(FOR[n_for]).setOp2(view.PrincipalGUI.AL_Instr.size());
					SemanticoInstrucao lc6 = new SemanticoInstrucao();
					lc6.setSeq(24);
					lc6.setCod("AMEN");
					lc6.setOp1(0);
					lc6.setOp2(-1);
					view.PrincipalGUI.AL_Instr.add(lc6);
					n_for -= 3;
					b = true;
				} catch (Exception e) {
					Erro(i);
	...
	public static void Erro(int j) {
		switch (j) {
		case 1:
			PrincipalGUI.Erro_Sem.add("O identificador já foi declarado - " + (model.Semantico.i + 1) + "° token");
			break;
		case 2:
			PrincipalGUI.Erro_Sem.add("O identificador não foi declarado - " + (model.Semantico.i + 1) + "° token");
			break;
		case 3:
			PrincipalGUI.Erro_Sem
					.add("É possivel atribuir valores apenas a varáveis - " + (model.Semantico.i + 1) + "° token");
			break;
		case 4:
			PrincipalGUI.Erro_Sem.add("Você não pode usar um rótulo declarado em um nível diferente - "
					+ (model.Semantico.i + 1) + "° token");
			break;
		case 5:
			PrincipalGUI.Erro_Sem.add("Esse identificador não é um rótulo - " + (model.Semantico.i + 1) + "° token");
			break;
		case 6:
			PrincipalGUI.Erro_Sem.add("Esse rótulo já foi usado - " + (model.Semantico.i + 1) + "° token");
			break;
		case 7:
			PrincipalGUI.Erro_Sem
					.add("Esse rótulo não aponta para nenhum lugar - " + (model.Semantico.i + 1) + "° token");
			break;
		case 8:
			PrincipalGUI.Erro_Sem
					.add("Esse identificador não é uma procedure - " + (model.Semantico.i + 1) + "° token");
			break;
		case 9:
			PrincipalGUI.Erro_Sem.add(
					"A procedure possui um número de parametros diferente - " + (model.Semantico.i + 1) + "° token");
			break;
		case 100:
