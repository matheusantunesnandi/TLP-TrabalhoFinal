...
public class LexicoGTabela {
	...
	public LexicoGTabela() {
		int n = PrincipalController.AL.size();
		for (int i = 0; i < n; i++) {
			r = 0;
			int m = LexicoTipoToken.AL_p.size();
			for (int j = 0; j < m; j++) { // Identifica se o token é uma palavra reservada
				if (PrincipalController.AL.get(i).toLowerCase().equals(LexicoTipoToken.AL_p.get(j).getNome())) {
					...
					PrincipalController.ALfinal.add(t);
					j = m;
					r = 1;
				}
			}
			if (r == 0) {
				...
				for (int j = 0; j < m; j++) { // Identifica se o token é um operador
					if (PrincipalController.AL.get(i).equals(LexicoTipoToken.AL_o.get(j).getNome())) {
						LexicoToken t = new LexicoToken();
				...
				if (r == 0) {
					...
					for (int j = 0; j < m; j++) { // Identifica se o token é um simbulo especial
						if (PrincipalController.AL.get(i).equals(LexicoTipoToken.AL_s.get(j).getNome())) {
							LexicoToken t = new LexicoToken();
					...
					if (r == 0) {
						if (Character.isDigit(PrincipalController.AL.get(i).charAt(0))) { // caso seja um inteiro
							LexicoToken t = new LexicoToken();
							t.setCodigo(26);
							t.setDesc("Inteiro");
							t.setNome(PrincipalController.AL.get(i));
							PrincipalController.ALfinal.add(t);
							r = 1;
						} else {
							if (PrincipalController.AL.get(i).charAt(0) == '\"') { // caso seja um literal
								LexicoToken t = new LexicoToken();
								t.setCodigo(48);
								t.setDesc("Literal");
								t.setNome(PrincipalController.AL.get(i));
