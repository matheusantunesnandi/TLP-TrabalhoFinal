...
public class Lexico {
	...
	public Lexico() {
		try {
			String t = PrincipalGUI.styledText.getText();// O código será guardado em "t"
				...
				case 1:// caso o token seja uma letra
					if (p.length() >= 30) {
						JOptionPane.showMessageDialog(null,
								"Palavras podem conter no máximo 30 caracteres (linha " + (ent + 1) + ")");
						i = n;
					} else {
						p = p + a;
						if (i == n - 1) {
							PrincipalGUI.AL.add(p);
							p = "";
						} else {
							a = t.charAt(i + 1);
				...
				case 2:// monta números
					...
					if (i == n - 1) {
						...
						if (Character.isDigit(a)) {
							...
							if (Character.isLetter(a)) {
								JOptionPane.showMessageDialog(null,
										"Uma palavra não pode começar em número (linha " + (ent + 1) + ")");
								i = n;
							} else {
								if (a == ':' || a == '>') {
									c = 3;
